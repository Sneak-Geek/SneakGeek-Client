...
import { View, StyleSheet, SafeAreaView, ScrollView, Text, Dimensions, TextInput, TouchableOpacity, Platform } from 'react-native';
    ...
    private _renderImage() {
        return (
            <View style={styles.swipercontainer}>
                ...
                >
                    <View style={{ backgroundColor: 'red', flex: 1, height: Dimensions.get('window').width - 40, borderRadius: 4 }} />
                    <View style={{ backgroundColor: 'yellow', flex: 1, height: Dimensions.get('window').width - 40, borderRadius: 4 }} />
                    <View style={{ backgroundColor: 'green', flex: 1, height: Dimensions.get('window').width - 40, borderRadius: 4 }} />
                    <View style={{ backgroundColor: 'aqua', flex: 1, height: Dimensions.get('window').width - 40, borderRadius: 4 }} />
                </Swiper>
            </View>
        )
    }
    ...
    private _renderContent() {
        return (
            <View style={styles.middleContainer}>
                <Text style={styles.shoeName} numberOfLines={1} ellipsizeMode="tail">NMD_R1 'Speckle Pack - white'</Text>
                <View style={styles.row}>
                    <View>
                        <Text style={styles.title}>Giá đăng</Text>
                        <Text style={styles.curency}>VND <Text style={styles.money}>1.600.000</Text></Text>
                    </View>
                    <View>
                        <Text style={[styles.title, { textAlign: 'right' }]}>Giá cao nhất hiện tại</Text>
                        <Text style={[styles.curency, { textAlign: 'right' }]}>VND <Text style={[styles.money, { textAlign: 'right' }]}>1.600.000</Text></Text>
                    </View>
                </View>
    ...
    private _renderInputMoney() {
        return (
            <View style={styles.inputMoneyContainer}>
                ...
                <View style={{ height: 28, padding: 5, backgroundColor: 'rgba(26, 188, 156, 0.1)', flexDirection: 'row', alignItems: 'flex-end', borderBottomWidth: 1, borderColor: Assets.Styles.AppPrimaryColor }}>
