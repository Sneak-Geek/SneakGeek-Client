...
export class AccountTabEditProfile extends React.Component<Props, State> {
    ...
    this.sectionList = [
      {
        ...
        sectionFields: [
          ...
          },
          {
            ...
            value: (profile: Profile): string => profile.userProvidedPhoneNumber,
            onUpdate: (value: string, profile: Profile): Profile => {
              return Object.assign(profile, {
      ...
      },
      {
        ...
        sectionFields: [
          ...
          },
          {
            ...
            value: (profile: Profile): string => profile.userProvidedAddress?.city,
            onUpdate: (value: string, profile: Profile): Profile => ({
              ...profile,
              userProvidedAddress: {
                ...profile.userProvidedAddress,
          ...
          },
          {
            ...
            value: (profile: Profile): string => profile.userProvidedAddress?.ward,
            onUpdate: (value: string, profile: Profile): Profile => ({
              ...profile,
              userProvidedAddress: {
                ...profile.userProvidedAddress,
  ...
  private _initializeAddressSettings(): void {
    const settingsService = getService<ISettingService>(FactoryKeys.ISettingService);
        ...
        {(headerHeight): JSX.Element => (
          ...
          >
            ...
            <Icon
              ...
              type={'feather'}
              size={themes.IconSize}
              onPress={(): void => this.setState({ editMode: !this.state.editMode })}
            />
          </View>
        )}
      </HeaderHeightContext.Consumer>
          ...
          <TouchableWithoutFeedback
            ...
            onPress={(): void => {
              if (item.isPicker && this.state.editMode) {
                this.setState({ pickerVisible: true, pickerType: item.pickerType });
              }
            }}
          >
  ...
  private _renderSetting(item: Setting): JSX.Element {
    ...
    if (editMode && !item.isPicker) {
      ...
      return <AppText.Subhead style={styles.input}>{textValue}</AppText.Subhead>;
    }
