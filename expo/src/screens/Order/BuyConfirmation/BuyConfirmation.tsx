...
class UnconnectedBuyConfirmation extends React.Component<Props, State> {
      ...
      <SafeAreaView style={{ flex: 1, justifyContent: 'flex-end' }}>
        <View style={{ flex: 1, backgroundColor: 'white' }}>
          ...
          <ScrollView style={{ flex: 1 }}>
            <View style={{ padding: 20 }}>
              <View style={{ flex: 1, flexDirection: 'column' }}>
                <OrderSectionWithTitle title={strings.ShoeSize} value={this.size} />
          ...
          <BottomButton
            ...
            title={this.state.isBuyNow ? strings.BuyProduct : strings.SetBuyPrice}
          />
        </View>
      </SafeAreaView>
    );
  ...
  private _renderOrderConfirmationDetail(): JSX.Element {
    ...
    return (
      <View>
        ...
        />
        <OrderSectionWithTitle title={strings.Address} value={address.join('\n')} />
  ...
  private _renderOrderTypeSelector(): JSX.Element {
    ...
    return (
      <View style={styles.orderTypeContainer}>
        <TouchableOpacity
          style={[!isBuyNow ? styles.orderTypeSelected : {}, styles.commonOrderType]}
          onPress={(): void => {
  ...
  private async _onPurchaseButtonClicked(): Promise<void> {
    if (!this.state.isBuyNow) {
      this.props.toggleLoading(true);
      try {
        await this.orderService.createBuyOrder(getToken(), this.state.newBuyOrder);
