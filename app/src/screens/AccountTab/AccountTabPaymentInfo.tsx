        ...
        if(currentScreen === Screen.PAYMENT_INFO){
            if(shouldContinue){
                try{
                    ...
                    Alert.alert("Tiền sẽ được chuyển vào tài khoản của bạn trong vòng 5 đến 10 ngày tới.");
                }catch(error){
                    Alert.alert("Lệnh rút tiền đang được thực hiện. Xin vui lòng liên hệ với SneakGeek nếu bạn cần thay đổi thông tin thanh toán.")
                }
                ...
                if(this.state.currentBalance === 0)
                    Alert.alert("Ví SneakGeek chưa có tiền.")
                if(this.state.withdrawalAmount > this.state.currentBalance)
                    Alert.alert("Số tiền rút ra không thể lớn hơn số tiền hiện có.")
                else{
                    Alert.alert("Xin vui lòng điền đủ thông thin chuyển khoản.")
                }
